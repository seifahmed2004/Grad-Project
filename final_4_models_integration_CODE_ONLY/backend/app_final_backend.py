...
# Keep website assets working if your new web uses /assets/...
...
def _normalize_video_for_model(input_path: Path, target_fps: int = 30) -> Path:
    ...
    This function rewrites the video as MP4 with stable target_fps.
    """
...
def _clean_repeated_words(text: str) -> str:
    """
    Removes direct repeated words only.
    Example: 'I I want breakfast' -> 'I want breakfast'
    """
    ...
    for word in words:
        if not clean_words or clean_words[-1].lower() != word.lower():
...
def _apply_sentence_fallback(result: dict) -> dict:
    ...
    if existing_text:
        full_sentence = _clean_repeated_words(str(existing_text).strip())
    ...
    for seg in segments:
        ...
        if word:
            # Avoid direct duplicates in sentence result
            if not fallback_words or fallback_words[-1].lower() != str(word).lower():
                fallback_words.append(word)

        if gloss:
            if not fallback_glosses or fallback_glosses[-1].upper() != str(gloss).upper():
...
def _get_mediapipe_holistic():
    """
    Supports MediaPipe legacy solutions.
    Works with mediapipe 0.10.14 in Python 3.10.
    """
    ...
    try:
        ...
        raise ImportError(
            "MediaPipe legacy solutions are required. "
            "Use: conda activate grad_py310 then python main.py"
        ) from exc


def _quick_hand_visible_check(video_path, max_frames=12):
    """
    Optional fast check for live mode.

    IMPORTANT:
    This check is NOT enabled by default in /api/sign/single.
    It only runs when frontend sends check_hand=true.

    That keeps the old website and normal prediction safe.
    """
...
def live_page():
    ...
    return JSONResponse(
        ...
        content={
            "ok": False,
            "error": "frontend/live.html not found yet."
        }
    )
...
def sign_single(
    ...
    # Keep False by default so normal web prediction never breaks.
    # Live frontend can send check_hand=true if it wants no_hand response.
...
# ===============================
# 4) Multi-Sign Video → Sentence → Speech
# Supports both:
# /api/sign/to-speech
# /api/sign/tts
# ===============================
...
def sign_to_speech(
    ...
    # Default True:
    # Multi-sign video -> sentence -> speech
    # If your frontend wants old single-sign behavior, send sentence_mode=false.
    ...
    try:
        ...
        if sentence_mode:
            ...
            sign_result = best_sign_result or {}
        ...
        text = _clean_repeated_words(str(text).strip())
        ...
        merged_result = {
            "pipeline": "multi_sign_video_to_sentence_to_speech",
